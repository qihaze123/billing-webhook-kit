...
Browser-only payment webhook fixture generator, raw-body signature verifier, payload inspector, entitlement decision matrix, duplicate replay simulator, debugging cost calculator, and review report exporter for SaaS billing integrations.
...
The free tool generates Lemon Squeezy webhook payloads, verifies HMAC signatures against exact raw request bodies, inspects trusted billing fields, recommends idempotency keys, maps events to entitlement decisions, simulates duplicate webhook replays, estimates webhook debugging cost, exports PR-ready Markdown review reports, creates cURL replay commands, and provides starter handlers for Next.js, Hono, and Express. It is designed for local billing integration tests, CI fixtures, and webhook replay debugging.
